...
FRCF_CSV_URL = (
    ...
    "/pub?gid=424283411&single=true&output=csv"
)


@app.route("/api/frcf")
def api_frcf():
    """Fetch FRCF session data from published Google Sheets CSV."""
    try:
        import requests as req
        resp = req.get(FRCF_CSV_URL, timeout=15)
        ...
        for row in reader:
            sessions.append({
                ...
                "session_date": row.get("Session Date", ""),
                "location": row.get("Location", ""),
                "client_id": row.get("Client ID #", ""),
                "age": row.get("Age", ""),
                "type_of_service": row.get("Type of Service", ""),
                "presenting_problem": row.get("Presenting Problem", ""),
